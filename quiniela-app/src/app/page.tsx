...
function getWhatsAppLink(user?: UserState) {
  const base = "https://wa.me/17542991555";

  const message = user?.fullName
    ? `Hola, tengo una duda sobre mi quiniela (${user.fullName})`
    : "Hola, tengo una duda sobre la quiniela";

  return `${base}?text=${encodeURIComponent(message)}`;
}

function WhatsAppSupportButton({ user }: { user?: UserState }) {
  return (
    <a
      href={getWhatsAppLink(user)}
      target="_blank"
      rel="noopener noreferrer"
      className="fixed bottom-5 right-5 z-[9999] flex items-center gap-2 rounded-full border border-white/10 bg-emerald-500 px-4 py-3 text-sm font-black text-white shadow-[0_18px_45px_rgba(16,185,129,0.35)] transition hover:bg-emerald-600 active:scale-[0.98] sm:bottom-6 sm:right-6 sm:px-5"
  ...
  useEffect(() => {
    ...
    const loadPredictions = async () => {
      ...
      if (shouldAutoFillMissing) {
        const missingRows = MATCHES.filter((match) => {
        ...
        if (missingRows.length > 0) {
          ...
          if (autoZeroError) {
            console.error(
              "Error aplicando ceros automáticos:",
              autoZeroError.message,
            );
          } else {
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-none text-sm leading-6 text-white/70 md:text-base">
              A continuación ingresa tu pronóstico de marcador para cada uno de
              los partidos. Mucha suerte. Tus cambios se guardan
              automáticamente.
            </p>
            ...
            <div className="mt-4 max-w-none rounded-2xl border border-red-400/20 bg-red-400/10 p-4">
              <p className="text-sm leading-6 font-semibold text-red-300">
                ⚠️ Te recuerdo que tienes hasta 2 horas antes de que empiece el
                partido inaugural.
              </p>
              <p className="mt-2 text-sm leading-6 font-bold text-red-400 underline">
                Después de esta hora, todos los marcadores que no hayas metido
                automáticamente se tendrán por “0”.
              </p>
            </div>
          </div>
        ...
        <div className="mt-6 rounded-3xl border border-yellow-400/30 bg-yellow-400/10 p-5 text-center shadow-[0_0_30px_rgba(250,204,21,0.12)]">
          ...
          <p className="mt-2 text-sm text-white/65">
            De este monto, {formatCurrencyMXN(PRIZE_CONTRIBUTION_PER_ENTRY)}{" "}
            integran la bolsa y {formatCurrencyMXN(ADMIN_FEE_PER_ENTRY)} son
            gastos de administración.
          </p>

          {activeEntryPaymentStatus === "paid" ||
          activeEntryPaymentStatus === "exempt" ? (
            <div
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  alert(
                    "Error iniciando el pago. Revisa la terminal del servidor.",
                  );
                }
              }}
          ...
          {isGlobalLock && (
            <>
              <p className="mt-3 text-sm font-bold text-red-400 underline">
                La captura de pronósticos ya está cerrada. Todos los marcadores
                no ingresados se tomarán como “0”.
              </p>
              <p className="mt-2 text-xs font-semibold text-red-300">
                Los ceros automáticos aparecerán en rojo para distinguirlos de
                los ceros capturados manualmente.
              </p>
            </>
          )}
        </div>
            ...
            <section key={groupName}>
              ...
              <div className="space-y-4">
                {matches.map((match) => {
                  ...
                  const locked =
                    isGlobalLock || getTimeLock(match.kickoff) || !state.isOpen;
...
function AdminScreen({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const loadPaymentEntries = async () => {
      ...
      setPaymentEntries(
        ((data ?? []) as AdminPaymentEntryRow[]).sort((a, b) => {
          const nameA =
    ...
    >
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 space-y-8">
          ...
          <section
            id="admin-payments"
            className="scroll-mt-8 mt-8 rounded-3xl border border-yellow-400/30 bg-gradient-to-br from-yellow-400/10 via-white/[0.03] to-black p-6 shadow-xl md:p-8"
          >
...
function EntryDetailScreen({
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      if (picksError) {
        console.error(
          "Error cargando predictions de la quiniela:",
          picksError.message,
        );
        setPredictionsMap({});
      } else {
...
export default function Home() {
  ...
  const canViewPublic =
    user?.role === "admin" || new Date() >= PUBLIC_REVEAL_DATE;
          ...
          .upsert(
            {
              ...
              role: isForcedAdmin
                ? "admin"
                : (storedProfile.role ?? profile.role ?? "player"),
            },
            { onConflict: "id" },
          )
          .select("*")
  ...
  const handleDeleteActiveEntry = async () => {
    ...
    if (refreshError) {
      alert(
        `La quiniela se borró, pero hubo un error al refrescar: ${refreshError.message}`,
      );
      return;
    }
  ...
  const activeEntryPendingAmount = Math.max(
    ENTRY_PRICE - activeEntryPaymentAmount,
    0,
  );
  ...
  if (view === "admin") {
    ...
    return (
      <>
        <AdminScreen onBack={() => openView("dashboard")} />
        <WhatsAppSupportButton user={user} />
      </>
    );
  }
    ...
    <main className="min-h-screen bg-black px-6 py-8 text-white md:px-10">
      <div className="mx-auto max-w-7xl">
              ...
              <div>
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`shrink-0 rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] ${getPaymentStatusClass(activeEntryPaymentStatus)}`}
                    >
                      {getPaymentStatusLabel(activeEntryPaymentStatus)}
                    </span>
                  </div>
        ...
        <section className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 md:items-stretch">
          ...
          {/* IZQUIERDA 1 */}
          <div
            className={`${user.role === "admin" ? "order-2" : "order-1"} md:order-1 md:contents`}
          >
            <DashboardCard
              title="Mi Quiniela"
          ...
          {/* DERECHA 1 */}
          <div
            className={`${user.role === "admin" ? "order-3" : "order-4"} md:order-2 md:contents`}
          >
            <DashboardCard
              title="Tabla general"
          ...
          {/* IZQUIERDA 2 */}
          <div
            className={`${user.role === "admin" ? "order-4" : "order-2"} md:order-3 md:contents`}
          >
            <DashboardCard
          ...
          {/* DERECHA 2 */}
          <div
            className={`${user.role === "admin" ? "order-6" : "order-5"} md:order-4 md:contents`}
          >
            <DashboardCard
          ...
          {/* IZQUIERDA 3 */}
          <div
            className={`${user.role === "admin" ? "order-7" : "order-3"} md:order-5 md:contents`}
          >
            <DashboardCard
          ...
          {/* DERECHA 3 */}
          <div
            className={`${user.role === "admin" ? "order-5" : "order-6"} md:order-6 md:contents`}
          >
            <DashboardCard
